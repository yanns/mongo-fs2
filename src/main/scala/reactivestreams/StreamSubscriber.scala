...
/** Implementation of a `org.reactivestreams.Subscriber`.
  *
  * This is used to obtain a `fs2.Stream` from an upstream reactivestreams
  * system.
  *
  * @see
      ...
      in match {
        ...
        case OnNext(a) => {
          ...
          case o =>
            o -> (() =>
              reportFailure(
                new Error(s"received record [$a] in invalid state [$o]")
              )
            )
        }
        case OnComplete => {
        ...
        case OnDequeue(r) => {
          ...
          case o =>
            o -> (() =>
              r(new Error(s"received request in invalid state [$o]").asLeft)
            )
        }
      }

    F.delay(new AtomicReference[(State, () => Unit)]((Uninitialized, () => ())))
      .map { ref =>
        new FSM[F, A] {
